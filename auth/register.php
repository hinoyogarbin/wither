...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['fullname'], $_POST['email'], $_POST['password'])) {
    ...
    $name = $_POST['fullname'];       // form fullname → DB name
    $email = $_POST['email'];
    ...
    if($stmt->execute()){
        echo "<script>alert('Account created successfully!'); window.location='../auth/login.php';</script>";
        exit;
    } else {
        echo "<script>alert('Error creating account'); window.history.back();</script>";
    }
} else {
    header("Location: ../auth/login.php"); // direct access without POST
    exit;
}
?>
